...
export default function ShortsViewer({ startId }: { startId?: string }) {
  ...
  // первичная загрузка: рандомный старт (или с заданного id)
  useEffect(() => {
    let on = true;
    fetch("/api/shorts").then((r) => r.json()).then((d) => {
      ...
      if (startId) {
        ...
        setIdx(items.length ? Math.floor(Math.random() * items.length) : 0);
      }
      setList(items);
    }).catch(() => setList(startId ? [{ id: startId, title: "", thumbnail: "" } as Video] : []));
    return () => { on = false; };
  }, [startId]);
  ...
  // бесконечность: подгружаем ещё, когда близко к концу
  useEffect(() => {
    if (loadingMore.current || idx < list.length - 4 || list.length === 0) return;
    loadingMore.current = true;
    const exclude = list.map((v) => v.id).slice(-120).join(",");
    fetch(`/api/shorts?exclude=${encodeURIComponent(exclude)}`).then((r) => r.json()).then((d) => {
      const have = new Set(list.map((v) => v.id));
      const fresh = (d.items || []).filter((v: Video) => !have.has(v.id));
      if (fresh.length) setList((prev) => [...prev, ...fresh]);
      ...
      <div className="short-stage">
        ...
        <div className="short-rail">
          ...
          <div className="short-navs">
            ...
            <div className="short-count">{idx + 1}</div>

...
const EMPTY = {
  ...
  searches: [],
  notInterested: [],
  subscriptions: {}, // channelId -> { title, thumbnail, importedAt }
};
...
export function recordEvent(event) {
  ...
  if (type === "subscribe") {
    const { channel } = event;
    if (channel?.id) {
      p.subscriptions[channel.id] = {
        title: channel.title || channel.id,
        thumbnail: channel.thumbnail || "",
        importedAt: event.at || new Date().toISOString(),
      };
      // подписка = сильный сигнал интереса к каналу
      bump(p.channels, channel.id, 8);
    }
  }
...
export function getSubscriptions() {
  const p = load();
  return Object.entries(p.subscriptions || {}).map(([id, info]) => ({ id, ...info }));
}

// Множество уже просмотренных id (для исключения из ленты).
export function watchedIds() {
  const p = load();
  return new Set(p.watchHistory.map((v) => v.id));
}
